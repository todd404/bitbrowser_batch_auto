...
By default, Playwright runs the browsers in headless mode. To see the browser UI, set [headless](https://playwright.dev/python/docs/api/class-browsertype#browser-type-launch-option-headless) option to `False`. You can also use [slow_mo](https://playwright.dev/python/docs/api/class-browsertype#browser-type-launch-option-slow-mo) to slow down execution. Learn more in the debugging tools [section](https://playwright.dev/python/docs/debug).
