...
Source: https://playwright.dev/python/docs/getting-started-mcp
...
ActionHow to do it**Install server**Add standard config to your MCP client**Navigate to a page**Ask: "Go to [https://example.com](https://example.com)"**Click an element**Ask: "Click the Submit button"**Fill a form**Ask: "Fill in the email field with [test@example.com](mailto:test@example.com)"**Take a screenshot**Ask: "Take a screenshot of the page"**Run Playwright code**Ask: "Run this Playwright code: ..."**Mock an API**Ask: "Mock the /api/users endpoint to return ..."**Use headed mode**Default. Pass `--headless` to disable**Choose a browser**Pass `--browser=firefox` in args
